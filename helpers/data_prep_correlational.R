...
# Function to prepare correlational data
prepare_correlational_data <- function(file_path = "data/Dataset_Correlations.xlsx") {
  
  # Load dataset
  Dataset_Correlations <- read_excel(file_path)
  data_main <- Dataset_Correlations
  ...
  data_main$ESRAW_Z <- as.numeric(data_main$ESRAW_Z)
  data_main$ES_VAR_Z <- as.numeric(data_main$ES_VAR_Z)
  
  return(data_main)
}

# Function to perform outlier analysis for correlational data
perform_outlier_analysis_corr <- function(data_main) {
...
# Initialize correlational data
init_correlational_data <- function(file_path = "data/Dataset_Correlations.xlsx") {
  data_main <- prepare_correlational_data(file_path)
  outlier_result <- perform_outlier_analysis_corr(data_main)
